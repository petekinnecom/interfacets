...
# Change the name
name_input = browser.dom.one("input")
...
Interfacets allows you to define component contracts using JSON-schema in a central `config/interfacets/components.yml` file. These contracts are automatically enforced in your Ruby tests.

### Features

- **Component Prop Validation**: Automatically fails tests if incorrect property types are sent to React components.
- **Event Trigger Validation**: Fails tests if an event is triggered with data that doesn't match the expected schema.
- **Strict Enforcement**: All components rendered in tests must have a corresponding entry in the registry.
- **Shared Definitions**: Supports `$ref` and shared schemas for complex definitions.

### Setup

1. Add `json_schemer` to your `Gemfile`:
   ```ruby
   gem "json_schemer"
   ```

2. Create `config/interfacets/components.yml` and define your schemas:
   ```yaml
   definitions:
     style:
       type: object
...
Validation is active for a browser session when you pass the `contract_path` to the initializer.

```ruby
browser = Interfacets::Test::UiSimulator.new(
  bus: server_bus,
  router: router,
  type: :inline,
  contract_path: "config/interfacets/components.yml"
)

# Raises Interfacets::ValidationError if 'label' is missing
browser.dom.one("TextField", value: "pete")

# Raises Interfacets::ValidationError if payload is invalid
browser.dom.one("TextField").trigger("onChange", { wrong: "payload" })
```

### Error Handling

- `Interfacets::ValidationError`: Raised when data does not match the schema.
- `Interfacets::MissingComponentContractError`: Raised when a component is encountered that is not in the registry.
